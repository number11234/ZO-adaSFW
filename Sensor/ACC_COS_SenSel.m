function [x_ave_track, eps_track, obj_track] = ACC_COS_SenSel(options)
...
mu = 1/(sqrt(d)*IterMax^(2/3));     % 平滑参数
B1 = 50;
B2 = 50 ;      % 批次


eta = 1/(IterMax^(2/3)) ;     % 学习率
...
for t = 1 :IterMax    %% 总迭代次数
    ...
    if t > 1
            ...
            if mod(t,50) == 0    %% 每50打印一次信息
               disp(sprintf('ACC_COS for iter = %d with xeps = %4.5f, obj = %4.5f',...
