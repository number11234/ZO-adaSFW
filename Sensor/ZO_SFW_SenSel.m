function [x_ave_track, eps_track, obj_track, obj_track_50] = ZO_SFW_SenSel(options)
    ...
    for t = 1 :IterMax    %% 总迭代次数
        lr = 1/(IterMax^(3/4)); % 学习率
        mu = 2/((t+8)^(1/2)*d^(1/2));    % 平滑参数
        beta = 1/(t+8)^(1/2);  % 动量系数
        ...
        if t > 1
                ...
                if mod(t,50) == 0    %% 每50打印一次信息
                   disp(sprintf('ZO_SFW for iter = %d with xeps = %4.5f, obj = %4.5f',...
