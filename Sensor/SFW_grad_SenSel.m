function [x_ave_track, eps_track, obj_track, obj_track_50] = SFW_grad_SenSel(options)
    %SFW_grad 此处显示有关此函数的摘要
    %   此处显示详细说明
    A_allT = options.data;
    ...
    for t = 1 :IterMax    %% 总迭代次数
        ...
        if t > 1
                ...
                if mod(t,50) == 0    %% 每50打印一次信息
                   disp(sprintf('SFW_grad for iter = %d with xeps = %4.5f, obj = %4.5f',...
